...
public class MVP implements Bot{
	...
	public int getCommand(Player player, Board board, Dictionary dictionary) {
		...
		//uncomment following block if dictionary check is to be implemented
/*
		if (!challengeMade) {
			challengeMade = true;
...
*/
		...
		if (board.getSqContents(CENTER, CENTER) == ' ') {
		...
		for (int intersectionRow = 0; intersectionRow < board.SIZE; intersectionRow++) {
			for (int instersectionCol = 0; instersectionCol < board.SIZE; instersectionCol++) {
				if ((tempChar = board.getSqContents(intersectionRow, instersectionCol)) != BLANK
					...
					switch (playType = lookForDirection(intersectionRow, instersectionCol, board)) {
	 ...
	 */
	private int lookForDirection(int i, int j, Board board) {
		int row = i;
		int col = j;
		int[] urdlI = { -1, 0, +1, 0 };
		int[] urdlJ = { 0, +1, 0, -1 };
